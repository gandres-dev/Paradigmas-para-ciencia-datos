...
p <- ggplot(economics, aes(x=date, y=psavert)) + geom_line(color='#0F61F1', size=1)
dev.off()
p
...
p + geom_line(aes(x=date, y=movil, color='Movil Media')) + geom_line(aes(x=date, y=expo, color = 'Media Exp.'))
